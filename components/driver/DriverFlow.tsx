...
export function DriverHome() {
  ...
  const mapProps = useMapProps('driver');

  return (
    <div className="flex h-full min-h-0 flex-col gap-3 overflow-y-auto p-3 scrollbar-thin md:p-4 lg:grid lg:grid-cols-[1fr_320px] lg:gap-4 lg:overflow-hidden">
      <div className="flex min-h-[200px] flex-col gap-3 lg:min-h-0">
        ...
        <MapPanel
          {...mapProps}
          height="min-h-[180px] flex-1"
          className="overflow-hidden rounded-2xl border border-white/[0.06]"
        />
      </div>

      <div className="flex flex-col gap-3 lg:overflow-y-auto lg:scrollbar-thin">
...
export function DriverPickup() {
  ...
  const mapProps = useMapProps('rider', 15);
...
export function DriverInTrip() {
  ...
  const mapProps = useMapProps('trip', driverState.progress);
  const ride = driverState.acceptedRide;
